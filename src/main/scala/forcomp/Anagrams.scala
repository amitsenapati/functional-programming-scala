...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val grouped = lowerCase.groupBy(c => lowerCase.count(c == _)).toList
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def product(xss: List[Seq[(Char, Int)]]): Seq[Seq[(Char, Int)]] = xss match {
      case Nil => List(Nil)
      case h :: t => {
        for {
          xh <- h
    ...
    val test = product(occurrences.map(allOccurences)).map(p => {
      p.filter(removeZero).toList
    })

    test.toList
  }

  def removeZero(p: (Char, Int)) = p._2 != 0
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    }).filter(removeZero).toList.sorted
  }
