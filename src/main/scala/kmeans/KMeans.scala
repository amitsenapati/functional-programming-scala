...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val x = points.map(p => (p, findClosest(p, means))).groupBy(_._2).map {case (k,v) => (k, v.map(_._1))}
    means.map(mean => (mean, x.getOrElse(mean, GenSeq[Point]()))).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    val x = classified.map { case (k,v) => (k, findAverage(k, v))}
    oldMeans.map(mean => x.getOrElse(mean, mean))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    for((o, n) <- oldMeans zip newMeans){
      if(o.squareDistance(n) > eta) return false
    }
    true
  }
